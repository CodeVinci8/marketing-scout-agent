...
// REPORT-TRUTH-C: the send sits behind the «XLSX Ready?» gate so a skipped workbook still reaches the
// completion edit. F-2 DELIVERY-LIFECYCLE-001: the whole workbook tail is now gated on a CONFIRMED text send
// («Report Delivered?»), so an undelivered report never spawns an orphan Excel upload.
A.ok('bundle -> report-confirmed gate -> XLSX -> ready gate -> send wired',
  hasEdge(w20, 'Append report_bundles', 'Report Delivered?') && hasEdge(w20, 'Report Delivered?', 'Build Report XLSX', 0)
  && hasEdge(w20, 'Build Report XLSX', 'XLSX Ready?') && hasEdge(w20, 'XLSX Ready?', 'Send Report XLSX', 0));
