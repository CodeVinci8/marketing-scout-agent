...
const EXPECTED_CLOSURE = [
  ...
  '09_avito_classifieds_listing_connector.json', '10_competitor_audience_intelligence_aggregator.json',
  '11_social_source_connector_foundation.json', '12_market_intelligence_report_builder.json',
...
A.eq('runtime closure is exactly the 17 expected workflows', closure, EXPECTED_CLOSURE.slice().sort());
A.eq('runtime-count == 17', L.deployment().runtime_workflow_count, 17);
// 16 = 13 (DEC-161) + Stage 5 fan-out (WF20 -> WF09 Avito / WF11 Telegram / WF26 VK).
A.eq('binding edges == 16', L.bindingEdges().length, 16);
A.eq('callable targets == 13', L.callableTargets().length, 13);
...
A.eq('import order is a permutation of the closure (17, unique)', order.length === 17 && new Set(order).size === 17, true);
...
A.eq('callable_trigger_count == 13', L.deployment().activation.callable_trigger_count, 13);
...
rejects('rejects a runtime closure of 16 (dropped workflow)', d => { d.runtime_closure = d.runtime_closure.slice(0, 16); d.runtime_workflow_count = 16; });
...
rejects('rejects a binding target outside the runtime closure', d => { d.binding_edges[0].target_workflow = '15_live_source_run_logger.json'; });
rejects('rejects an import order that is not a permutation of the closure', d => { d.import_order = d.import_order.slice(0, 16); });
rejects('rejects a test-only workflow injected into the runtime set', (d, m) => {
  d.runtime_closure = d.runtime_closure.concat(['15_live_source_run_logger.json']); d.runtime_workflow_count = 18;
  const e = (m.workflows || []).find(w => w.file === '15_live_source_run_logger.json'); if (e) e.deployment_role = 'manual_diagnostic';
});
