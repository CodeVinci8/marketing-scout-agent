...
// =============================================================================================================
A.section('1. resolver — exactly 46 resolved sheets, ordered, non-empty unique headers');
A.eq('DECLARED_TABS=46', resolved.summary.DECLARED_TABS, 46);
A.eq('RESOLVED_HEADER_SETS=46', resolved.summary.RESOLVED_HEADER_SETS, 46);
A.eq('UNRESOLVED_HEADER_SETS=0', resolved.summary.UNRESOLVED_HEADER_SETS, 0);
A.eq('sheet_count=46', resolved.sheet_count, 46);
...
A.ok('sheet names are unique', new Set(resolved.sheets.map(function (s) { return s.sheet_name; })).size === 46);
A.ok('sheet_order is 1..46 contiguous', resolved.sheets.every(function (s, i) { return s.sheet_order === i + 1; }));
...
A.eq('dry-run plans 46 creates (preview)', dry.summary.CREATED_TABS, 46);
...
A.eq('creates exactly the 46 missing contract sheets', a1.create_requests.length, 46);
...
A.eq('one basic filter per contract sheet', a1.requests.filter(function (r) { return r.setBasicFilter; }).length, 46);
...
A.section('3b/5b. order — contract sheets occupy first 46, extras preserved AFTER in relative order');
...
A.ok('first 46 visible sheets are the contract sheets in contract order', JSON.stringify(ordered.slice(0, 46)) === JSON.stringify(resolved.sheets.map(function (s) { return s.sheet_name; })));
A.eq('extras follow contract sheets, original relative order preserved', ordered.slice(46), ['zeta_extra', '__qa_connection', 'alpha_extra']);
...
A.ok('all 46 contract sheets exist, frozen, ordered, with exact headers', resolved.sheets.every(function (cs, i) {
...
A.eq('CONTRACT_TABS_ORDERED=46', a1.summary.CONTRACT_TABS_ORDERED, 46);
