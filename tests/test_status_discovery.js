...
A.section('workflow_inventory matches the REAL manifest runtime set (17 workflows, unique names)');
{
  const identity = L.runtimeIdentity();
  const names = Object.keys(identity).map(k => identity[k].name);
  A.eq('17 runtime workflows', names.length, 17);
  A.eq('all runtime names unique', new Set(names).size, 17);
  ...
  const r = INV.classify(identity, entries);
  A.eq('matched=17 for a clean install', r.summary.matched, 17);
...
A.section('STATUS-001 e2e — --status classifies a real listing (no false "(not imported)")');
{
  ...
  A.ok('reports 17 exact matches', /INVENTORY_MATCHED=17/.test(r.out));
...
A.section('STATUS-001 e2e — a renamed runtime workflow is reported as renamed, not missing/not-imported');
{
  ...
  const r = withStubN8n('2.23.3', listing, null);
  A.ok('16 exact matches', /INVENTORY_MATCHED=16/.test(r.out));
...
// ---------------------------------------------------------------------------------------------------------------
// OPERATOR-REPORT-001 — binding counts in operator output derive from the manifest (13 today), never hard-coded "8".
A.section('OPERATOR-REPORT-001 — operator binding counts derive from the manifest, not a hard-coded 8');
{
  ...
  A.eq('manifest binding edge count', edges, 16);
