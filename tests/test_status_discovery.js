...
A.section('workflow_inventory matches the REAL manifest runtime set (15 workflows, unique names)');
{
  const identity = L.runtimeIdentity();
  const names = Object.keys(identity).map(k => identity[k].name);
  A.eq('15 runtime workflows', names.length, 15);
  A.eq('all runtime names unique', new Set(names).size, 15);
  ...
  const r = INV.classify(identity, entries);
  A.eq('matched=15 for a clean install', r.summary.matched, 15);
...
A.section('STATUS-001 e2e — --status classifies a real listing (no false "(not imported)")');
{
  ...
  A.ok('reports 15 exact matches', /INVENTORY_MATCHED=15/.test(r.out));
...
A.section('STATUS-001 e2e — a renamed runtime workflow is reported as renamed, not missing/not-imported');
{
  ...
  const r = withStubN8n('2.23.3', listing, null);
  A.ok('14 exact matches', /INVENTORY_MATCHED=14/.test(r.out));
