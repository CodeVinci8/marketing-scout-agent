...
- **3.3 — First real source connector: Avito/Classifieds Listing Connector** 🔧 **BUILT + RELEVANCE FILTER; FIXTURE + HANDOFF PASS; LIVE TRANSPORT PASS, LIVE RELEVANCE RETEST PENDING (2026-06-11, DEC-090/092/093/094/095)**: live retest #2 (`avito_req_20260611_001222`, actor `fatihtahta~avito-russia-scraper`) returned 10/10 valid items — **transport works** — but the 2 unique rows were legal-address **false positives** → **DEC-095 v005 business relevance filter** (relevance from title/description/decoded-slug/category only, query never qualifies a listing; hard negatives → `hard_skipped`, never written, filtered before `pipeline_limit=10`; 8-count summary; canonical URLs). **Stage 3.3 closes after a clean live run (Test 3b).** Earlier (2026-06-10, DEC-090/092): WF09 fixture tests + the WF08 deterministic handoff passed (monitor 5 / skipped 1 / technical_errors 0 / Claude 0); **fixture-mode only — no real Avito scrape** (`fixture_mode=true`, `live_mode=false`, source cost $0). **DEC-092 quality patch:** WF09 `max_items=6` (= total fixture records), richer `service_hint`/`semantic_keywords`, Apify live node wired for HTTP Header Auth (no secrets); WF08 deterministically enriches Avito competitor rows (offer/title, price/terms, specific service theme, content_idea_score 35–55, competitor_strength 75–85) — gated to WF09-origin rows, Stage 3.2 baseline unchanged. Live Apify smoke (actor id + bound credential, `max_items=3–5`) still pending.
...
- **3.5 — WF10 Market Intelligence Aggregator (Competitor Semantic & Ad Intelligence aggregation)** 📐 **PLAN WRITTEN (2026-06-11, DEC-099)** — `docs/WF10_COMPETITOR_AUDIENCE_INTELLIGENCE_AGGREGATOR_PLAN.md`; **not built**, gated on at least one stable live source. WF08 = "what is this one record?"; WF10 = "what is happening in the market overall?" — windows 7/14/30 days over monitor/content/review queues into `competitor_profiles` / `market_angles` / `audience_activity_signals` / `content_positioning_plan` / `source_confidence_rules`. Original scope: aggregate competitor
