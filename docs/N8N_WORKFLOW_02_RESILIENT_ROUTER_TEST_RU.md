...
## Статус тестов A–E (прогон 2026-06-06)

| Тест | Результат | Комментарий |
|---|---|---|
| A — горячий лид | ✅ PASS | route=results, parse_method=primary_json |
| B — слабый лид | ⚠️ PASS после патча | До патча уходил в `content_queue`. Исправлено (DEC-036): слабый/потенциальный лид теперь идёт в `review_queue` раньше, чем в content_queue. **Требуется повторный прогон Теста B.** |
| C — конкурент | ✅ PASS | route=monitor_queue. `company_name` был пустым → добавлен описательный fallback (`МФО / частный кредитор` и т.п.) |
| D — Markdown→repair | ✅ PASS | Валидирует Repair Formatter. `service_type="займ под залог ПТС"` теперь нормализуется в `pts_loan` |
| E — нечитаемый ответ | ✅ PASS | Валидирует ветку `technical_errors`, needs_manual_review=true |

**Что делать оператору:** повторно прогнать **Тест B** (убедиться route=review_queue), затем по желанию быстрый smoke Тестов A и D. Тесты C и E повторно прогонять не нужно — их ветки логики не менялись.

Изменён только узел **Normalize + Route** (приоритет маршрутизации + нормализация service_type + fallback company_name). Промпты, модель и архитектура repair не менялись. Расход API за прогон A–E: **$0.0750** (было $0.1145 → стало $0.1895).
