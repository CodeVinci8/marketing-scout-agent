...
## 11e. Полный E2E (4 URL) + усиление PTS-override и извлечение контактов (DEC-066)

**Сквозной тест прошёл (2026-06-07).** WF05 (запрос «автоломбард Москва займ под ПТС без проверки кредитной
истории») → WF06 выбрал 4 одобренных `direct_competitor` → WF04 обработал все 4 → все в `monitor_queue`,
`entity_type=competitor`, `recommended_action=monitor`, `processing_status=parsed_success`. Но **две страницы**
получили `service_type=generic_lending`, хотя это явно ПТС:
- `autolombardn1.ru/` (текст: «займы под залог ПТС», «автоломбард», «любая КИ», «ПТС», «транспорт»);
- `autolombard-moskva.ru/services/.../dmitrovskoe-shosse/` («займ под залог ПТС», «автоломбард»,
  «Дмитровское шоссе», «ставка от 2%», «без проверки КИ»).

**Патч (DEC-066, нода `Normalize + Route`, две части):**

1. **Усиленный финальный override PTS (B3).** После прежней цепочки B2: если `entity_type=competitor` и в
   URL+тексте присутствует **≥3** различных сильных токена из набора
   (`птс`, `залог птс`, `под птс`, `автоломбард`, `залог авто`, `залог автомобил(я)`, `авто остаётся`,
   `любая ки`, `без проверки кредитной истории`/`ки`) → `service_type=pts_loan`. **Кроме** случаев: настоящий
   **мультипродуктовый корень** (остаётся `generic_lending`) и **явно только недвижимость** (остаётся
   `secured_real_estate_loan`). Проверено симуляцией: `autolombardn1.ru`→`pts_loan`,
   `autolombard-moskva.ru/services/…`→`pts_loan`, `mosinvestfinans.ru/`→`generic_lending`,
   lioncredit (недвижимость)→`secured_real_estate_loan`.

2. **Извлечение/санитизация контактов.** `contact_public` ценен — валидные контакты **не удаляются**.
   Новая логика `bestContact(model, text)`: сначала санитизируется значение модели (`sanitizeContact`),
   и если оно валидно — **сохраняется целиком**; иначе детерминированно извлекается из `text_context`
   (`extractContactFromText`: телефон RU 10–11 цифр `+7`/`8`/`7`, ссылка `t.me/`, `wa.me/`, email).
   - Сохраняются: `"+7 495 740-01-01, https://t.me/autolombardvip, https://wa.me/79857375386"`,
     `"8 800 777-95-67; https://lk.cashmotor.ru/"`.
   - Гасятся: `"+7 (495) ... (номер указан на сайте, требуется извлечение)"` (есть `...`/`требуется
     извлечение` и нет полного контакта) → пусто; при наличии реального контакта в тексте он подставляется.
   - **Предпочтение** реальному извлечённому контакту над выдуманным моделью «частичным».

**Без изменений:** ровно **35** бизнес-колонок, **10** колонок `url_registry`, архитектура дедупа.

**Статус:** ✅ Workflow 04 — **одобренный потребитель URL** с усиленным PTS-override и хардненгом контактов.

---

## 11f. Финальный хардненг Stage 2 — расширенный PTS-override и детерминированное извлечение контактов (DEC-070)

Финальный проход перед одобрением Stage 2. Усилены обе части ноды `Normalize + Route`:

1. **Расширенный набор сильных PTS-токенов (B3).** Добавлены: `эптс`, `займ под залог птс`,
   `автомобиль остаётся`/`машина остаётся`, `любая кредитная история` (к уже бывшим `птс`, `под птс`,
   `залог птс`, `автоломбард`, `залог авто`, `залог автомобиля`, `без проверки ки`/`кредитной истории`,
   `любая ки`). Правило прежнее: `entity_type=competitor` + **≥3** различных токена в
   `text_context + offer_text + terms + reason` (+ URL) → `service_type=pts_loan`, **кроме**
   мультипродуктового корня (`generic_lending`) и явной «только недвижимости» (`secured_real_estate_loan`).
   Чистое авто без ПТС по-прежнему может стать `secured_auto_loan`.

2. **Детерминированное извлечение контактов `extractContacts()` + `bestContact(model, text, source_url)`.**
   Извлекаются только надёжные публичные контакты: телефон RU (10–11 цифр после очистки; `+7`/`7`/`8`/`8-800`),
   email, Telegram (`@handle` ≥4 символов или `t.me/`), WhatsApp (`wa.me/`), и контактные/профильные/
   заявочные URL — **но не сам `source_url` страницы**. Сначала извлечение из значения модели (валидный полный
   контакт сохраняется, частичный отбрасывается), иначе — из текста страницы; **детерминированный контакт
   предпочтительнее «частичного» от модели**. Телефонный/almail/handle-скан идёт по копии без URL, поэтому
   цифры внутри `wa.me/<номер>` не считаются отдельным телефоном.
   - **Сохраняются:** `+7 495 740-01-01`, `8 800 777-95-67`, `https://t.me/autolombardvip`,
     `https://wa.me/79857375386`, `@username`, валидный email.
   - **Гасятся:** `+7 (495) ...`, `номер указан на сайте`, `телефон есть на сайте`, `требуется извлечение`,
     любое значение с многоточием без полного надёжного контакта → `contact_public=""`.

**Без изменений:** ровно **35** бизнес-колонок, **10** колонок `url_registry`, архитектура дедупа. Юнит-тест
логики контактов и симуляция PTS-override пройдены. См. `docs/STAGE_2_WEB_PIPELINE_REVIEW.md` (тест-матрица).
