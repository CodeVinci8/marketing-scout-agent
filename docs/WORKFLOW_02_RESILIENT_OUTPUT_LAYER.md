...
> **Reused by Workflow 03 (2026-06-07, DEC-039–042):** `n8n/workflows/03_firecrawl_single_url_resilient.json` (Firecrawl single-URL scraper) **reuses this resilient output layer**. Its analyzer nodes (`Build Primary Claude Request`, `Claude Primary API Request`, `Parse Primary JSON`, `IF Primary Parse OK?`, `Build Repair Request`, `Claude Repair API Request`, `Parse Repaired JSON`, `Normalize + Route`, `Append to Dynamic Route Sheet`) are **copied** from the production workflow so Workflow 03 stays standalone and importable (no sub-workflow call yet). The only changes: the source record now comes from `Normalize Firecrawl Output` (the three `$('Set Source Record')` lookups became `$('Normalize Firecrawl Output')`), and a Firecrawl failure emits a 33-field `technical_errors` row that bypasses Claude entirely. Routing, repair, normalization, and the 33-column schema are unchanged.

> **Post-repair consistency hardening in Workflow 03 (2026-06-08, DEC-043/044):** The first real Firecrawl test (`mosinvestfinans.ru/`) exposed that a **structurally valid `repaired_json`** can still carry contradictory business values (competitor with `competitor_strength=1`, `quality_score=6`, `recommended_action=investigate`, a Chinese-language `reason`, `service_type=pts_loan` on a multi-product page) and was mis-routed to `review_queue`. Workflow 03's `Normalize + Route` now **distrusts repaired output and enforces business invariants** for scraped website pages: competitor signal-count floors (≥3→65, ≥5→75), rich competitor → `monitor_queue` (never `review_queue`), a `reason` language guard (Russian fallback when the source is Cyrillic but `reason` is CJK/foreign), multi-product → `generic_lending`, and a compact `raw_response_preview` (200 chars) on `parsed_success`. This is **n8n Code-node logic only** — no prompt change, no extra API call, still 33 fields. This hardening currently lives in Workflow 03; fold it back into the Workflow 02 production node when Workflow 03's path is confirmed.
