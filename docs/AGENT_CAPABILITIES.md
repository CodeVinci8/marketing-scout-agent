...
- **Touchpoint Analyzer (Workflow 08, Stage 3.2 — DEC-080, PATCHED v3 DEC-082)** 🔧 BUILT, PATCHED, UNDER RETEST — `08_touchpoint_analyzer.json` (JSON valid, `active=false`). **Source-agnostic, DETERMINISTIC-FIRST** analyzer: reads approved/unique `raw_market_records`, classifies each record **deterministically from intake hints** and appends to the 6 business tabs (existing **35-column** schema) via dynamic `Sheet Name = {{ $json.route }}`. **TEST 2 (v2) was ROUTING PASS but LLM-stability FAIL + cost-efficiency FAIL** (`primary_json=0`, ≈$0.159 Claude for 12 records while the deterministic floor did the classification). **Patched (DEC-082):** `Set Analyzer Config` adds `analysis_mode='deterministic_first'` + `llm_enrichment=false`; an LLM gate (`call_claude = NOT irrelevant AND (llm_enrichment OR deterministic_needs_llm)`) routes obvious records through **`Build Deterministic Row`** with **no Claude call** (`deterministic_pre_route` / `deterministic_irrelevant_skip`, $0). Claude is **optional enrichment, OFF by default**; when called it keeps the primary→repair→deterministic-fallback chain. Prompts hardened (no browse/fetch, no "fetching/checking/analyzing", one JSON object); `Normalize + Route` collapses `market_signal`→`content_idea`, scales 1–10 scores to 1–100 with a deterministic floor. Encoded safeguards retained: hot lead without usable contact → `review_queue` (not auto-contact). **No scraping, no Apify/Firecrawl; writes only business tabs.** Expected default-mode retest: Claude calls=0, `deterministic_pre_route=10`, `deterministic_irrelevant_skip=2`, `technical_errors=0`. Guide: `docs/N8N_WORKFLOW_08_TOUCHPOINT_ANALYZER_RU.md`; plan: `docs/STAGE_3_2_TOUCHPOINT_ANALYZER_PLAN.md`; test log: `docs/STAGE_3_2_TEST_RESULTS.md`.
