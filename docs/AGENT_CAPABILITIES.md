...
- **Touchpoint Analyzer (Workflow 08, Stage 3.2 — DEC-080, PATCHED v2 DEC-081)** 🔧 BUILT, PATCHED, UNDER RETEST — `08_touchpoint_analyzer.json` (20 nodes, JSON valid, `active=false`). **Source-agnostic** analyzer: reads approved/unique `raw_market_records`, analyzes each record with Claude using the **Stage 2 resilient JSON/repair pattern**, maps the 12 touchpoint classes onto the existing **35-column** business schema, and appends to the 6 business tabs via dynamic `Sheet Name = {{ $json.route }}`. **First live test partially failed** (gateway returned prose/thinking, not JSON → classifiable records fell to `technical_errors`). **Patched (DEC-081):** computes a **deterministic classification from intake hints for every record** and uses it as a **fallback after LLM+repair failure** (`deterministic_fallback_after_llm_fail`); `technical_errors` only for truly unclassifiable records or Sheets/API errors; prompts/parser hardened (strict JSON; concatenate text items, ignore thinking/signature). Irrelevant skipped before Claude ($0). Encoded safeguards: `contact`→`results` needs lead_signal+score≥70+usable contact (forum record 11 → `review_queue`, not auto-contact). **No scraping, no Apify/Firecrawl; writes only business tabs.** Guide: `docs/N8N_WORKFLOW_08_TOUCHPOINT_ANALYZER_RU.md`; plan: `docs/STAGE_3_2_TOUCHPOINT_ANALYZER_PLAN.md`; test log: `docs/STAGE_3_2_TEST_RESULTS.md`.
