...
- **Touchpoint Analyzer (Workflow 08, Stage 3.2 — DEC-080)** 🔧 BUILT, UNDER TEST — `08_touchpoint_analyzer.json` (20 nodes, JSON valid, `active=false`). **Source-agnostic** analyzer: reads approved/unique `raw_market_records`, analyzes each record with Claude using the **Stage 2 resilient JSON/repair pattern** (primary → repair formatter → `technical_errors` fallback; `parse_method`/`repair_used`/`repair_status`/route validation), maps the 12 touchpoint classes onto the existing **35-column** business schema, and appends to the 6 business tabs via dynamic `Sheet Name = {{ $json.route }}`. **Irrelevant skipped deterministically before any Claude call ($0).** Encoded safeguards: `contact`→`results` needs lead_signal + score≥70 + usable contact (so the forum hot-pattern record routes to `review_queue`, not auto-contact); Avito competitor→`monitor_queue`; Dzen/VK/Telegram source candidates→review/content. **No scraping, no Apify/Firecrawl; writes only business tabs (not agent_requests/market_record_registry/agent_memory).** Guide: `docs/N8N_WORKFLOW_08_TOUCHPOINT_ANALYZER_RU.md`; plan: `docs/STAGE_3_2_TOUCHPOINT_ANALYZER_PLAN.md`.
