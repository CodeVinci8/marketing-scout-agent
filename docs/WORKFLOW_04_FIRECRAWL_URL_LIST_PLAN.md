...
>
> **contact_public sanitation patch (DEC-063):** the same E2E test stored `contact_public = "+7 (495) ... (номер указан на сайте, требуется извлечение)"` — a partial/placeholder, not a usable contact. `Normalize + Route` now runs `sanitizeContact()` on every `contact_public` emitter (main analyzed return + technical_error + deterministic-fallback pass-throughs): the value is **blanked** unless it matches a reliable public-contact pattern (phone `+7`/`8`/`7` with 10–11 digits, valid email, Telegram `@handle`/`t.me/`, or a contact/profile URL), and is blanked outright on `...`/`…` or `требуется извлечение`. **Never invent or keep a partial contact.** No field-count/dedup change; 35/10 intact.
>
> **Manual E2E result (2026-06-08):** WF05 discovered `https://carcapital.ru/` (`candidate_type=direct_competitor`, `service_hint=pts_loan`, confidence 100, `approval_status=approved`) → WF04 → `route=monitor_queue`, `entity_type=competitor`, `company_name=CarCapital`, `service_type=pts_loan` (post DEC-062), `competitor_strength=82`, `quality_score=85`, `processing_status=parsed_success`, `parse_method=primary_json`. The partial `contact_public` from that run is the case DEC-063 fixes.
