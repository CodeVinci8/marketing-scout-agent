...
write('19_request_planner.json', wf('19 — Request Planner (deterministic + guarded Claude)', [
  ...
  code('wf19-det', 'Deterministic Plan', [-40, 0], ['request_planner', 'llm_capability'],
    CALLER + "\nvar ci=callerInput();\nvar cfg0=$('Resolve Agent Config').first().json;\nvar tel=[];try{tel=($('Read llm_analysis_telemetry').all()||[]).map(function(x){return x.json;});}catch(e){tel=[];}\n// Fold the observed AI capability into the config so cost_model + the RU renderer tell the same truth.\nvar cfg=withClaudeCapability(cfg0,tel,{});\nvar text=String(ci.request_text||ci.text||'');\n// RECHECK-TARGET-001: a rerun/recheck (button callback 'intent:rerun_request' or NL «проверить изменения повторно»)\n// carries NO url in its text. Preserve the ORIGINAL target (do not degrade to a generic discovery) by seeding the\n// planner with the owner's most-recent registered source(s) from url_registry — the planner then re-checks the SAME\n// site as a change-report. A request that DOES name a source is unaffected (the planner ignores prior then).\nvar __rerun=/^intent:rerun_request$/.test(text)||/повтор|сделай снова|проверить изменени|что изменил|\\brerun\\b|\\bagain\\b/i.test(text);\nvar __prior=null;\nif(__rerun){var reg=[];try{reg=($('Read url_registry').all()||[]).map(function(x){return x.json;}).filter(function(r){return r&&(r.normalized_source_url||r.source_url);});}catch(e){reg=[];}var __own=String(ci.owner_user_id||'');var mine=reg.filter(function(r){return !__own||String(r.owner_user_id||'')===__own;});mine.sort(function(a,b){return String(b.last_seen_at||b.first_seen_at||'').localeCompare(String(a.last_seen_at||a.first_seen_at||''));});var __u=[];for(var __i=0;__i<mine.length&&__u.length<3;__i++){var __v=mine[__i].normalized_source_url||mine[__i].source_url;if(__v&&__u.indexOf(__v)<0)__u.push(__v);}if(__u.length)__prior={urls:__u};}\nvar plan=deterministicPlan(text,cfg,__prior?{prior:__prior}:{});\nreturn [{json:{request_text:text,rerun_recheck:__rerun,prior_target_count:(__prior&&__prior.urls?__prior.urls.length:0),agent_request_id:String(ci.agent_request_id||''),chat_id:String(ci.chat_id||''),owner_user_id:String(ci.owner_user_id||''),conversation_id:String(ci.conversation_id||''),data_mode:String(ci.data_mode||'live'),plan:plan,cfg:cfg,claude_capability:{available:cfg.claude_available,mode:cfg.claude_auth_mode,proof_at:cfg.claude_proof_at}}}];"),
...
return [{json:{analysis:Object.assign({},col,{reason:String(ai.analysis_reason||'disabled'),requested:(ai.targets||[]).length})}}];`),
  ...
  code('wf20-shapebundle', 'Shape Report Bundle', [1940, 60], ['analysis_report_ru', 'claim_validation'], `
var s=$('Build Execution Summary').first().json;
...
// REPORT-TRUTH-A: the mode travels in the bundle — every later renderer/export states WHICH question this
...
// the bundle — every later consumer (XLSX, WF24 export, Stage G) inherits truthful items. The audit persists in
// the bundle for observability.
try{var __ana=s.analysis||{};var __cv={analyses:__ana.analyses||[],audit:{}};
try{__cv=cvValidateAnalyses(__ana.analyses||[],cvBuildCtx({analyses:__ana.analyses||[],evidence:b.evidence||[]}));}catch(e){}
...
// AI-CONTRACT-001: the plan promised AI but none was delivered -> the «Аналитические выводы» sheet must SAY SO
// (never be silently empty). One honest inference row carries the verified reason; the collected facts stay intact.
try{var __aiExp=(s.summary&&s.summary.ai_expected)===true;var __aiDel=(s.summary&&s.summary.ai_delivered)===true;if(__aiExp&&!__aiDel){var __AIR={server_error:'сервис AI временно недоступен',overloaded_error:'сервис AI перегружен',rate_limit_error:'достигнут лимит запросов к AI',timeout:'AI не ответил вовремя',invalid_response:'AI вернул некорректный ответ',no_evidence:'недостаточно данных для AI-анализа',disabled:'AI-анализ отключён настройками',budget:'исчерпан бюджет на AI-анализ'};var __r=__AIR[String((s.summary&&s.summary.ai_error_category)||'')]||'AI-анализ не выполнен';var __note='AI-анализ не выполнен: '+__r+'. В отчёте — только собранные факты, без AI-интерпретации.';var __ax=(b.analysis&&typeof b.analysis==='object')?b.analysis:{};var __inf=Array.isArray(__ax.inferences)?__ax.inferences.slice():[];if(!__inf.some(function(x){return x&&String(x.text||'').indexOf('AI-анализ не выполнен')>=0;}))__inf.unshift({text:__note,markers:'',dimension:'ai_status'});b.analysis=Object.assign({},__ax,{inferences:__inf,ai_not_performed:true,ai_reason_ru:__r});}}catch(e){}
...
return [{json:{xlsx_skipped:false,chat_id:chat,caption:'Таблица Excel по анализу конкурентов',xlsx_filename:pkg.filename,xlsx_sheets:pkg.sheet_names},binary:{attachment:{data:Buffer.from(pkg.buffer).toString('base64'),fileName:pkg.filename,mimeType:pkg.mime}}}];`),
  ifNode('wf20-ifxlsx', 'XLSX Ready?', [2490, 160], '={{ $json.xlsx_skipped === false }}'),
...
var fs='completed',rec=0,an=0,ar=0,aix=false,aid=false,aer='';try{var s=($('Build Execution Summary').first().json||{}).summary||{};fs=String(s.final_state||'completed');rec=Number(s.records_reported)||0;an=Number(s.llm_analyses)||0;ar=Number(s.llm_analyses_reused)||0;aix=s.ai_expected===true;aid=s.ai_delivered===true;aer=String(s.ai_error_category||'');}catch(e){}
...
// REUSE-OBS-001: every invocation records an EXPLICIT cache decision (+reason), and a hit keeps the origin ref —
// Finalize forwards both so the summary/bundle/XLSX can prove where a reused result came from and why we did/didn't pay.
