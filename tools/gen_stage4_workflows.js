...
if(inp.domain==='memory'){
 ...
 if(inp.op==='none'){out.reply=mine.length?('Хорошо, ничего не добавляю. '+mine.length+' кандидат(ов) сохранены — можно вернуться к ним позже.'):'Хорошо, ничего не добавляю.';}
  ...
  // DISCOVERY-005 add policy: only in-region competitors above threshold; never aggregators/directories/news,
  // never a region mismatch, never already tracked. (Validated-only gate is added in DISCOVERY-006.)
  var pick=mine.filter(function(c){return truthy(c.is_competitor)&&!truthy(c.is_news_or_aggregator)&&String(c.region)!=='mismatch'&&Number(c.confidence||0)>=thr&&!truthy(c.already_tracked);});
