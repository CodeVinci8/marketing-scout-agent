...
return [{json:{analysis:Object.assign({},col,{reason:String(ai.analysis_reason||'disabled'),requested:(ai.targets||[]).length})}}];`),
  ...
  code('wf20-shapebundle', 'Shape Report Bundle', [1940, 60], ['source_role', 'analysis_report_ru', 'claim_validation'], `
var s=$('Build Execution Summary').first().json;
...
// WIP2b SOURCE-ROLE-001: classify each analyzed source's ROLE from its evidence (never the niche) so the report
// states role / relationship-to-niche / direct_competitor / confidence / limitations. Grouped by source_id/URL.
try{var __srcRoles={};(b.evidence||[]).forEach(function(e){var eu=String((e&&(e.url||e.profile_url))||'');var sid=String((e&&(e.source_id||e.source_key||e.competitor||e.source_name))||'')||eu;if(!sid)return;var g=__srcRoles[sid]||(__srcRoles[sid]={source_id:sid,source_url:eu,kind:String((e&&(e.source_kind||e.source_type))||''),evidence:[]});g.evidence.push({evidence_id:String((e&&e.evidence_id)||''),excerpt:String((e&&e.excerpt)||'')});});
var __roleRows=Object.keys(__srcRoles).map(function(sid){var r=__srcRoles[sid];var c=classifySourceRole({source_id:sid,kind:r.kind,niche:b.niche,evidence:r.evidence});return {source_id:sid,source_url:r.source_url,source_type:r.kind,source_role:c.source_role,role_confidence:c.role_confidence,role_reason:c.role_reason,direct_competitor:c.direct_competitor,evidence_ids:c.evidence_ids,relationship_to_niche:(c.source_role==='irrelevant_or_uncertain'?'релевантность не подтверждена':(c.direct_competitor?'прямой конкурент в нише':'публичный источник по теме ниши')),limitations:c.limitations};});
if(__roleRows.length)b.source_roles=__roleRows;}catch(e){}
