...
`server_error` от Anthropic — недетерминированно, форсировать нельзя. (2) «Липкий прогресс при ошибке
оркестратора» (WF20 падает без честного терминала; триггер здесь — квота Sheets) — **отдельный, более крупный
фикс** (error-branch в WF20), НЕ сделан в этой сессии. AI-CONTRACT-001 закрыт полностью.
