...
**Item 7 — восстановление `req_76722118` (219246148, упал 07-28 на квоте Sheets в WF12) — ВЫПОЛНЕНО.**
Данные восстановлены из сохранённого исполнения: узел «Build Deterministic Report» (exec 1444) отработал ДО
429 на «Append agent_requests», бандл `report_20260728_152153` сохранён. Отчёт по факту **no-data**
(competitors/offers/evidence = 0; sources_checked=104 → 0 профилей; `do_analyze=false`) → XLSX не полагается.
Каноническим рендером (`compact_report_ru`) собрано честное сообщение и **доставлено вживую в чат 219246148
(Ruslan): Telegram `message_id=719`, `ok=true`** (по явному разрешению оператора; токен из env контейнера,
token-safe, БД не дешифровалась). Без нового плана, без платного сбора, без AI, без дубля (реюз сохранённых
данных, одно сообщение). Прежний блокер (auto-mode classifier харнеса на живую отправку реальному
пользователю) снят разрешением оператора.
...
19 импортов, бэкап `/root/backups/n8n-prerelease-20260725-131917/` (SHA записан), `PRODUCTION_SOURCE_PARITY=PASS`
19/19, биндинги 18×noop. Маркер релиза `ABORTED` на политике активации из-за предсуществующего активного legacy
`WF14` — импорты персистированы. Затем publish-callables (15) + активация WF18 + пересоздание контейнера.
...
**Git + деплой (выполнено).** PR #52 смёржен в `main` (`71a2e74`) после зелёного CI. **Хирургический деплой**
(не whole-runtime): `deploy_workflow_jscode.js` — графт jsCode на живой прод-экспорт WF20/WF24 (набор узлов и
топология идентичны, id/active/креды/связи/webhookId сохранены). Бэкап ДО импорта:
`/home/node/.n8n/predeploy-backup-20260729-173828/` (в томе n8n; wf20_prod.json/wf24_prod.json — точка отката).
Импорт → рестарт n8n → republish WF20/WF24 (callable-таргеты). **Пост-деплой (read-only):** `health=true,
active=17, registered_webhooks=1, ingress_post=200, RUNTIME INTEGRITY: OK`; новый код подтверждён в проде
(`Progress: Done` содержит `delivered_no_ai`, `Build Execution Summary` — `ai_expected`); **WF27/WF28/WF03/WF07
не тронуты**. Откат: повторный `import:workflow` из бэкапа + republish.

**Пользователь `219246148` (Ruslan).** Доставка отчёта+XLSX подтверждена прод-БД (exec 1477). Провалившийся
`req_76722118` (07-28) упал не из-за AI, а на **транзиентной квоте Google Sheets** в WF12 (exec 1444) → прогресс
завис (~7/10). **Реплей этого запроса заблокирован**: n8n REST требует авторизации (401), а извлекать
хешированный API-ключ/пароль владельца или слать пользователю синтетическое сообщение — недопустимо. Записано
как ограничение.

**Открытые ограничения (честно).** (1) Живое наблюдение терминала `delivered_no_ai` требует РЕАЛЬНОГО
`server_error` от Anthropic — недетерминированно, форсировать нельзя. AI-CONTRACT-001 закрыт полностью.

**TERMINAL-ON-ERROR-001 (сессия 78) — «липкий прогресс 7/10» закрыт.** Корневая причина `req_76722118`/exec 1439
установлена по прод-БД: WF12 упал на узле **«Append agent_requests» с httpCode=429** (per-minute квота Google
Sheets), а `Run WF12/16/08/10` в WF20 не имели обработки ошибок (`onError` по умолчанию = abort) → весь WF20
падал, прогресс замирал ~7/10, честного терминала не было. Исправление (канонично): `progress_tracker.js` —
состояние `run_failed` + `deliveryErrorEdit(ctx)` (причина по httpCode/сообщению: 429/quota/timeout/auth/generic;
никогда не «✅ Готово», не заявляет доставку, «запрос сохранён — можно повторить»); генератор — `execWf` поддержал
`errorOutput` (`continueErrorOutput`), у `Run WF16/08/10/12` включён error-выход, добавлены узлы «Progress: Failed
(Terminal)» + «Edit Progress (Failed)», error-выход (index 1) каждого узла → терминал-хендлер (success-выход 0 без
...
- **Read-only экспорт прод (90 workflow)** через `n8n export:workflow --all` (docker exec), скопирован в scratch,
  проанализирован офлайн; сырые экспорты после снятия производных фактов **удалены** (сохранён только
  санитизированный `DERIVED_COMPARISON.txt`: fingerprints/хеши/наборы имён — без сырых id/кредов/jsCode).
- **Прод-паритет ПОДТВЕРЖДЁН (не предположение).** Нормализованный logic-хеш: прод **WF20/WF24/WF25 ==
  `e96bf4a`** и отличается от `main` **только jsCode** узлов правдивости (набор узлов + топология идентичны);
  прод **WF27/WF28 == `main`** (дрейфа нет). Значит `main` действительно = прод + WIP1-изменения WF20/24/25.
- **Скан всех 90** (не только 5 предполагаемых) нашёл **предсуществующий дрейф вне scope**: неактивные
  ручные/тестовые **WF03** (1 code-узел `Normalize + Route`) и **WF07** (в проде 14 узлов против 16 — нет
  DEC-137 узлов `Append/Build live_source_runs Row`). Оба неактивны, не трогались сессией 75 — **над ними не
  деплоим**, отдельное решение оператора.
- **Путь деплоя выбран: хирургический `tools/deploy_workflow_jscode.js`** (не структурный). Офлайн dry-run:
  SURGICAL-CLEAN для всех трёх — меняется только заданный jsCode, сохраняются id/active/креды/связи/webhookId/
  sub-workflow-биндинги; изменённый jsCode == `main`. `backup.sh --dry-run` → `BACKUP_DEST_SAFE=PASS`
  (`mkdir` в проверке места — артефакт non-root, не дефект). `rollback.sh` dry-run → план ОК.
- **Правки правдивости** (перенесены свежей веткой `docs/post-gate-cleanup` от `origin/main`): офлайн-фикстуры
  vs живые исполнения; набор RU-доков добавлен, не консолидирован; убран раздел «## Оператор» из README;
  исправлено устаревшее «в работе, НЕ смёржено» и `== 8c1bd94` → текущий `8f4781e`; брендинг-каузинг репозитория
  → `vinci-ai-pilot`. Один локальный коммит, **НЕ запушен**. Классификация legacy-бренда «Marketing Scout Agent»:
  текущих продукт-facing вхождений не осталось (переименование уже сделано); остальные — историческое evidence
  или технические идентификаторы (креды/systemd/пути/схемы/встроенные промпты) по контракту `docs/BRANDING.md`.
- **Первое открытое: Stage F.5, Модуль 1 Opportunity Radar** — типизированные `opportunity_signals` со ссылками
  на `supporting_evidence_ids[]` (сигнал без доказательства не создаётся). Порядок Radar → Touchpoint → Action →
  Analyst. Плюс операторская добавка приёмки F.5: одна ограниченная живая VK-проверка перед гейтом F.5 (не сам
  F.5, не каноническое требование). Stage G/H — не трогаем.
...
- Три представительных сценария рендера проверены **офлайн по фикстурам** (НЕ точный ре-рендер оригинальных
  бандлей — сырые бандли 1370/1380/1390 лежат в контейнере n8n и недоступны только для чтения вне него):
  форма **сравнения** — на **реальном** сохранённом анализе `exec_1349` (source_id `multi`, метки `…+03:00`),
  использованном как фикстура (2/2/2, 0 дублей); **понижение** synthesis→сравнение (смешанный, 1/2/1, причина
  с carcapital.ru) и **синтез** (смешанный, fresh 1 ≠ contributing 3 ≠ вызовов 4, 1 квалифицированное заявление
  «подтверждено: autolombard-moskva.ru») — **честные реконструкции** представительных случаев. Все МСК-метки,
  RU-аспекты, без утечки «multi», рабочие URL. Живые исполнения 1370/1380/1390 остаются LIVE-PROVEN отдельно
  (сессия 74). Новой живой проверки в WIP1 не было ($0).

**Статус (итог сессии 75).** Все WIP закрыты и **смёржены в `main`**: PR #49 → `8c1bd94`, затем PR #50 →
**`8f4781e`** (текущий `origin/main`; `8c1bd94` был промежуточным) — после зелёного CI `offline-regression`
(и на ветке, и на `main` — первый зелёный `main` после красной серии #46–#48; финальный run `30130350678`).
Коммиты: `cb3c40a` (WIP1 правдивость), `647f56e` (WIP2 CI), `cd1f05b` (WIP3 Russian-документация),
`9bb1d15` (WIP4 регистрация 5 наборов, мёртвого кода нет). Полная офлайн-регрессия: **161 набор / 9215 assert /
0 провалов / внешних вызовов 0 / $0**. Фиче-ветка сохранена, рабочее дерево чистое. **Продакшн НЕ трогали.**

**Дрейф репозиторий → продакшн.** Относительно `e96bf4a` (крутится в проде) изменились только **WF20/WF24/WF25**
(WIP1-усиление правдивости); **WF27/WF28 без изменений**. Минимальный будущий деплой (с одобрения оператора):
структурный backup-first WF20/24/25 + `make verify-production` + паритет + runtime-integrity. Новых платных
живых прогонов и деплоя в этой сессии не было.
